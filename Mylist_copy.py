...
class UList(object):
	...
	def size(self):
		...
		"""
		value_count = 0
		temp_last = self.current
		while self.current != None:
			value_count += 1
			self.current = self.current.next
		self.current = temp_last
		return value_count

	def is_empty(self):
		""" 
			returns -1 if the List is empty else returns 1 
		"""
		return -1 if self.current == None else 1
	...
	def search(self, item):
		...
		loop_count = 0
		temp_last = self.current
		while self.current != None and not found:
			# loop_count += self.current.numvalue
			loop_count += 1
			if self.current.numvalue == item:
				found = True
			else:				
				self.current = self.current.next
		if found:
			# set the current item back to the last added item 
			self.current = temp_last
		return found
